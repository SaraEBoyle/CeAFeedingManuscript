function [gcamp, iso, baseline_gcamp, baseline_iso] = correct_bleaching_V2(imported_data, file, correct_for_bleaching, gcamp, iso, moving_avg, frame_rate, trigger1, smoot)
    gcamp_clone = gcamp;
    
    first_tau = 6; %1
    second_tau = 18; %3
    
    %% Find and use 1 minute before tray placement as baseline
    tray_column = [];
    %First find the tray placement column
    for p = 1:size(imported_data, 2)
        name = imported_data{1, p};
        if strcmp(name, 'new agar POINT')
            tray_column = p;
            break
        elseif contains(name, 'START')
            tray_column = horzcat(tray_column, p);
            
        end
    end
    
    % now collect 1 minute before tray placement as baseline
    baseline_gcamp = [];
    baseline_iso = [];
    tray_times = imported_data(3, tray_column);
    tray_times = cell2mat(tray_times);
    times = gcamp(:, 1);
    for tray_time = tray_times
        after = find(times >= (tray_time - 40));
        before = find(times <= (tray_time - 10));
        baseline_inds = intersect(after, before);
        baseline_gcamp = vertcat(baseline_gcamp, gcamp(baseline_inds, :));
        baseline_iso = vertcat(baseline_iso, iso(baseline_inds, :));
    end
    
    cropped_bleach = 1:size(baseline_gcamp, 1);
    if correct_for_bleaching
        avg_F = mean(gcamp(:,2));
        plot(gcamp(:,1),gcamp(:,2))
        temp_fit = fit(baseline_gcamp(cropped_bleach,1),baseline_gcamp(cropped_bleach,2),'exp2'); %find bi-exponential best fit for gcamp
        close all
    ...
    if correct_for_bleaching
        ...
        if trigger1
            ...
            if size_dif > 0
                temp_fit = fit(baseline_iso(cropped_bleach(1:(end - size_dif)),1),baseline_iso(cropped_bleach(1:(end - size_dif)),2),'exp2'); %find bi-exponential best fit for gcamp
            else
                temp_fit = fit(baseline_iso(cropped_bleach,1),baseline_iso(cropped_bleach,2),'exp2'); %find bi-exponential best fit for gcamp
            end
            close all
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if trigger1
        ...
        baseline_gcamp = green(baseline_inds);
        baseline_iso = violet(baseline_inds);
        save(horzcat(file, 'Corrected_data_full_session.mat'), 'gcamp', 'iso', 'baseline_gcamp', 'baseline_iso');
        hold off
        green_cor = green;
        violet_cor = violet;
    elseif trigger1
