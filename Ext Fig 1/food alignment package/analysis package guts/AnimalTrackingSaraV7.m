...
%% clear workspace
%% TODO make another function to recalculate z scores by using all baseline data

%%TODO classify time spent in the corners
function [] = AnimalTrackingSaraV7(bleaching_correction, timing_data, imported_data, pre_times, post_times, baseline_times, control_fiber, signal_fibers, fiber_names, photometry_data)
%This is a list of all possible events to align to- do not change it
 signal_key_word = 'signal';

%% photometry variables
if bleaching_correction == 1
    ...
    for fiber = signal_fibers
            ...
            if photometry_data == 1
                ...
                for signal = 1:2
                    ...
                    [photo_data] = load_data(signal_key_word, file, photometry_data);
                    if signal == 1
                        fiber_title = horzcat(fiber_names{fiber}, ' signal');
                        %to get signal data
                        %Trim your data if needed
                        [photo_data, auto_flo] = trim_data(photo_data, fiber, 1);

                        %De-interleave and subtract autofluorescence
                        [gcamp, iso] = de_interleave(1, 0, file, fiber_title, photo_data, fiber, auto_flo, 1, signal);

                        %correct bleaching
                        workingdir = horzcat(file, fiber_title, '\');
                        [gcamp, iso, baseline_gcamp, baseline_iso] = correct_bleaching_V2(imported_data, workingdir, correct_for_bleaching, gcamp, iso, moving_avg, frame_rate, 1, 0);

                    elseif signal == 2
                        %to get control channel
                        fiber_title = horzcat(fiber_names{fiber}, ' control');
                        if fiber == control_fiber(fiber)
                            ...
                            %correct bleaching
                            workingdir = horzcat(file, fiber_title, '\');
                            [gcamp, iso] = correct_bleaching(horzcat(workingdir, fiber_title, '\'), correct_for_bleaching, gcamp, iso, moving_avg, frame_rate, 1, 0);
                    ...
                    %for each type of event
                        ...
                        %For each event type align
                        for event_type_ind = 1

                            alignment = event_name;     

                            if ~exist(horzcat(file, fiber_title), 'dir')
                                mkdir(horzcat(file, fiber_title));
                                mkdir(horzcat(file, fiber_title, '\', alignment));
                            elseif ~exist(horzcat(file, fiber_title, '\', alignment))
                                mkdir(horzcat(file, fiber_title, '\', alignment));
                            end
                            filesub = horzcat(file, fiber_title, '\', alignment, '\');
                            cd(filesub);
                            ...
                            if plot_heatmap_and_average
                                ...
                                if ~isempty(times_delivered)
                                    ...
                                    baselines = [];
                                    if ~contains(alignment, 'STOP')
                                        for r = 1:size(US_trials, 1)
                                            cur = US_trials(r, :);
                                            baseline = cur(1:((baseline_time)*fps_photometry));
                                            baselines(r, :) = baseline;
                                        end
                                    else
                                        for r = 1:size(US_trials, 1)
                                            cur = US_trials(r, :);
                                            baseline = cur((end - ((baseline_time)*fps_photometry)): end);
                                            baselines(r, :) = baseline;
                                        end
                                    end
                                ...
                                if 1
                                    ...
                                    if isempty(times_delivered)
                                        ...
                                        baseline_US_trials = US_trials(:,1:(baseline_time*fps_photometry)); %baseline_time*fps_photometry instead it's all time before US
                                    end

                                    if(~isempty(times_delivered))
                                        %%baseline_trials = vertcat(baseline_US_trials);
                                        %% TODO USE BASELINES FROM BLEACHING CORRECTION baseline_gcamp/iso
