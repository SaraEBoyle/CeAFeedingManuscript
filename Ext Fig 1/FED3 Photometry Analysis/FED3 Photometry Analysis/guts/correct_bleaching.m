function [gcamp, iso] = correct_bleaching(file, correct_for_bleaching, gcamp, iso, moving_avg, frame_rate, trigger1, smoot, pellet_delivered_times)
    gcamp_clone = gcamp;
    gcamp_minus_events = gcamp;
    baseline_inds = [];
    for e = 1:size(pellet_delivered_times, 1)
        time_scale = gcamp(:, 1);
        event = pellet_delivered_times(e);
        baseline_inds_left = find(time_scale >= (event - 15));
        baseline_inds_right = find(time_scale <= (event));
    ...
    cropped_bleach_gcamp = 1:size(gcamp, 1);
    first_tau = 6; %1
    second_tau = 18; %3
    if correct_for_bleaching
        %gcamp = gcamp_minus_events;
        avg_F = mean(gcamp(:,2));
        plot(gcamp(:,1),gcamp(:,2))
        temp_fit = fit(gcamp(cropped_bleach_gcamp,1),gcamp(cropped_bleach_gcamp,2),'exp2'); %find bi-exponential best fit for gcamp
        close all
    ...
    elseif moving_avg
        ...
        avg_mat = tsmovavg(roiDATA(:,:),'s',tau1,1); %Finds simple moving average over the dataset

        for t = 1:size(roiDATA,1)
            roi_fzero(t,:) = min(avg_mat(max(1,t-tau2):t,:)); %Gets the minimum value of the sliding window within range of tau2
        end 
    ...
    plot(gcamp(:,1),gcamp(:,2));
    %% Find fit using times without signal
    cropped_bleach_gcamp = 1:size(gcamp_minus_events, 1);
    cropped_bleach_iso = 1:size(gcamp, 1);
    %cropped_bleach = cropped_bleach(1:(end - 1));
    if correct_for_bleaching
        plot(gcamp(:,1),gcamp(:,2))
        temp_fit = fit(gcamp_minus_events(cropped_bleach_gcamp,1),gcamp_minus_events(cropped_bleach_gcamp,2),'exp2'); %find bi-exponential best fit for gcamp
        close all
        plot(gcamp(:,1),gcamp(:,2)) %plot data and fit on same graph to make sure fit looks ok
        hold on 
        plot(temp_fit);
        ...
        close all
        if trigger1
            size_dif = size(cropped_bleach_iso, 2) - size(iso, 1);
            
            if size_dif > 0
                temp_fit = fit(iso(cropped_bleach_iso(1:(end - size_dif)),1),iso(cropped_bleach_iso(1:(end - size_dif)),2),'exp2'); %find bi-exponential best fit for gcamp
            else
                temp_fit = fit(iso(cropped_bleach_iso,1),iso(cropped_bleach_iso,2),'exp2'); %find bi-exponential best fit for gcamp
            end
            close all
