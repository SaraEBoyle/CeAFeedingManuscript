...
for thing = 1:size(signal_paths, 2)
    ...
    for s = 3:size(thingy, 1)
        ...
        if or(contains(new_name, 'Entire_Session'), contains(new_name, 'raw_data'))
            continue
        end
...
signal_grandkids = {};
n = 1;
for p = 1:size(all_signal_paths, 2)
    grandkids = dir(all_signal_paths{p});
    if size(grandkids, 1) > 2
        for y = 3:size(grandkids, 1)
            signal_grandkids{n} = grandkids(y, 1);
            n = n + 1;
        end
    end
end
...
control_grandkids = {};
n = 1;
for p = 1:size(all_control_paths, 2)
    grandkids = dir(all_control_paths{p});
    if size(grandkids, 1) > 2
        for y = 3:size(grandkids, 1)
            control_grandkids{n} = grandkids(y, 1);
            n = n + 1;
        end
    end
end

for o = 1:size(signal_grandkids, 2)
    %Find data
    z_left_trials = [];
    left_trials = [];
    z_right_trials = [];
    right_trials = [];
    signal_path = signal_grandkids{:, o};
    signal_path = horzcat(signal_path.folder, '\', signal_path.name);
    control_path = control_grandkids{:, o};
    control_path = horzcat(control_path.folder, '\', control_path.name);
    ...
    if ~strcmp(is_good, 'n')
        ...
        save(horzcat(signal_path,'\artifacts_removed\dFF_trials.mat'), 'dFF_trials', 'avg_dFF_trials', 'dFF_control_trials', 'avg_dFF_control_trials', 'US_target_frame', 'x_axis', 'deleted_trials_inds')
